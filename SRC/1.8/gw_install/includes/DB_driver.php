 ...
 * @changes		Better debugging mode, query()
 * @changes		Possibility to skip queries when in debug mode
 * @changes		renamed common `log_message` into uncommon `ci_log_message`.
 * @changes		CI_DB_driver(), function names
 * @changes		load_rdriver(), path names
 * @changes		display_error()
 */
// ------------------------------------------------------------------------
...
class CI_DB_driver {
	...
	var $result_id		= FALSE;
	var $db_debug		= FALSE;
	var $db_debug_q		= FALSE; /* Do not run queries */
	var $benchmark		= 0;
	var $query_count	= 0;
	var $bind_marker	= '?';
	...
	function CI_DB_driver($params)
	{
		...
		ci_log_message('debug', 'Database Driver Class Initialized');
	}
		...
		if ($this->database != '')
		{
			if ( ! $this->db_select())
			{
				...
				{ 
					...
					{
						ci_log_message('error', 'Unable to create database: '.$this->database);
					...
					else
					{
						...
						if ($this->db_select())
						{
							...
							{
								ci_log_message('error', 'Unable to set database connection charset: '.$this->char_set);
				...
				ci_log_message('error', 'Unable to select database: '.$this->database);
	...
	function query($sql, $binds = FALSE, $return_object = TRUE)
	{
		if ($sql == '')
		{
			if ($this->db_debug)
			{
				ci_log_message('error', 'Invalid query: '.$sql);
		...
		if ($this->db_debug_q)
		{
			$this->queries[] = '<b>-.-----</b> '. htmlspecialchars( substr($sql, 0, 2048) ) . '... <i>' . strlen($sql) .' bytes</i>';
			return;
		}
		...
		if (FALSE === ($this->result_id = $this->simple_query($sql)))
		{
			...
			if ($this->db_debug)
			{
				...
				ci_log_message('error', 'Query error: '.$this->_error_message());
		...
		/* 26 june 2008: Limit the number of queries for debugging */
		if ($this->db_debug && $this->query_count < 100)
		{
			$this->queries[] = '<b>'.sprintf("%1.5f ", (($em + $es) - ($sm + $ss))).'</b>'. htmlspecialchars( substr($sql, 0, 2048) ) . "...\n\n<i style=\"color:#888\" dir=\"ltr\"><b>" . strlen($sql) .'</b> bytes</i>';
		}
	...
	function load_rdriver()
	{
		...
		{
			global $o;
			include_once($o->g('path_db').'/DB_result.php');
			include_once($o->g('path_db').'/'.$this->dbdriver.'/'.$this->dbdriver.'_result'.EXT);
...
#			include_once(BASEPATH.'database/drivers/'.$this->dbdriver.'/'.$this->dbdriver.'_result'.EXT);
	...
	function trans_complete()
	{
		...
		if ($this->_trans_status === FALSE)
		{
			...
			ci_log_message('debug', 'DB Transaction Failure');
			return FALSE;
		}
	...
	{
		$oE = new CI_Exceptions();
		print $oE->show_error('MySQL Error', $error, 'error_db');
		exit;
	}
	
	function old_display_error($error = '', $swap = '', $native = FALSE)
	{
		global $LANG;
		$LANG->load('db');
